...
- **Executor** (`trading_executor.py::TradingExecutor`) — consumes the queue and drives the full trade lifecycle per order:
  1. Fetch current price, size the order in `TRADE_SIZE_USD`, round to the symbol's `LOT_SIZE` step and validate against `minQty`/`MIN_NOTIONAL`. All filters + `baseAsset` are fetched in one shot per order via `_get_symbol_filters` (a single `get_symbol_info` call, not three) and fall back to conservative defaults — logged as a warning — if Binance errors or changes the filter shape.
  ...
  - `start()`'s consumer loop wraps each `execute_order` call in try/except: an unexpected exception on one order (malformed order dict, non-`BinanceAPIException` network error, etc.) is logged + reported but does not propagate — it would otherwise crash the whole process via `asyncio.gather` in `main.py`, taking the Telegram listener down with it.
  - Binance calls (`python-binance`'s `Client`) are synchronous and currently run inline inside the executor's coroutines, blocking the shared event loop for their duration — including the listener's polling — for as long as each Binance request takes. Not yet addressed; would need `run_in_executor` or a migration to `binance.client.AsyncClient`.
